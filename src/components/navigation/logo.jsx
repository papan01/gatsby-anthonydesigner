...
const AuthorProps = (index, hover) => {
  const delayArr = [0, 400];
  return {
    from: { opacity: 0, transform: index === 0 ? 'matrix(1.2,0,0,1,3,0)' : 'matrix(1.2,0,0,1,-21,0)' },
    to: { opacity: 1, transform: 'matrix(1,0,0,1,0,0)' },
    config: { easing: easings.easeCubicOut },
    delay: hover ? delayArr[index] : delayArr[(index + 1) % delayArr.length],
    reverse: !hover,
  };
};
...
function Logo() {
  ...
  const [authorProps, set] = useSprings(AuthorPaths.length, i => AuthorProps(i, hover));
    ...
    <Link to="/">
      ...
      >
        ...
        {authorProps.map(({ opacity, transform }, index) => (
          <animated.path key={AuthorPaths[index]} d={AuthorPaths[index]} style={{ opacity }} transform={transform} />
        ))}
      </svg>
    </Link>
  );
